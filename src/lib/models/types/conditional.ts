...
export class ConditionalType extends Type {
  ...
  stringify(wrapped: boolean): string {
    return wrap(
      wrapped,
      `${this.checkType.stringify(true)} extends ${this.extendsType.stringify(
        true
      )} ? ${this.trueType.stringify(true)} : ${this.falseType.stringify(true)}`
    );
  }
  ...
  serialize(
    ...
    init: BaseSerialized<ConditionalType>
  ): SerializedConditionalType {
    return {
      ...init,
...
export interface SerializedConditionalType
  extends Serialized<
    ConditionalType,
