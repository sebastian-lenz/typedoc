...
export interface SerializeWorker<T extends Type | Reflection> {
  ...
  serialize(
    value: T,
    serialized: Partial<ModelToObject<T>>
  ): Partial<ModelToObject<T>>;
}

export interface RegisterableSerializeWorker<T extends Type | Reflection>
  extends SerializeWorker<T> {
  kind: T["kind"];
}

export class Serializer {
  private _reflectionSerializers = new Map<
    ReflectionKind,
    SerializeWorker<SomeReflection>[]
  >();
  ...
  addReflectionSerializer(
    ...
    worker: SerializeWorker<SomeReflection>
  ): void {
  ...
  toObject<T extends SomeType | SomeReflection>(
    value: T,
    init: object = {}
  ): ModelToObject<T> {
    ...
    assert(
      serializers,
      `No serializers registered for object with kind: ${value.kind}. This is a bug.`
    );

    return serializers.reduce(
      (serialized, worker) => worker.serialize(value as any, serialized),
      init
    ) as ModelToObject<T>;
  }

  toObjects<T extends SomeType | SomeReflection>(
    values: T[]
  ): ModelToObject<T>[] {
    ...
    serialize(reflection, init) {
      ...
      if (reflection.comment) {
        base.comment = {
          ...
          tags:
            reflection.comment.tags?.map((tag) => ({
              tagName: tag.tagName,
              text: tag.text,
      ...
      if (reflection.isContainer()) {
        const children = serializer.toObjects<SomeReflection>(
          reflection.children
        );
