...
export class ObjectType extends Type {
  ...
  clone(): ObjectType {
    return new ObjectType(
      cloned(this.properties),
      cloned(this.signatures),
      cloned(this.constructSignatures)
    );
  }
  ...
  stringify(wrapped: boolean): string {
    ...
    if (
      ...
      !this.constructSignatures.length &&
    ...
    if (
      ...
      this.constructSignatures.length === 1
    ) {
  ...
  serialize(
    ...
    init: BaseSerialized<ObjectType>
...
export interface SerializedObjectType
  extends Serialized<
    ObjectType,
    "properties" | "signatures" | "constructSignatures"
  > {}
...
export class PropertyType extends Type {
  ...
  stringify(wrapped: boolean): string {
    assert(
      wrapped === false,
      "Property types are not considered wrapped when included in an object."
    );
  ...
  serialize(
    serializer: Serializer,
    init: BaseSerialized<PropertyType>
...
export interface SerializedPropertyType
  extends Serialized<
    PropertyType,
    "name" | "isReadonly" | "isOptional" | "propertyType"
  > {}
