...
export interface TypeConverter<T extends ts.Type = ts.Type, O extends M.SomeType = M.SomeType> {
    order?: number;
...
const literalTypeConverter: TypeConverter<ts.LiteralType, M.LiteralType> = {
    ...
    convert(_converter, type) {
        if (typeof type.value === 'object') {
            return new M.LiteralType({ value: type.value.base10Value, negative: type.value.negative });
        }
...
const referenceTypeConverter: TypeConverter<ts.Type, M.ReferenceType> = {
    ...
    convert(converter, type: ts.Type & { aliasSymbol: ts.Symbol }) {
        return new M.ReferenceType(type.aliasSymbol.name,
            type.aliasTypeArguments?.map(arg => converter.convertType(void 0, arg)) ?? [],
            type.aliasSymbol,
            false,
            converter.project);
    }
...
const tupleTypeConverter: TypeConverter<ts.TupleType, M.TupleType> = {
    supports(type) {
        return isObjectType(type) && Boolean(type.objectFlags & ts.ObjectFlags.Tuple);
    },
    convert(converter, type) {
...
function convertTypeParameters(converter: Converter, parameters: readonly ts.TypeParameter[]): M.TypeParameterType[] {
...
function convertParameters(converter: Converter, parameters: readonly ts.Symbol[]): M.SignatureParameterType[] {
    return parameters.map((symbol, index) => {
        const type = converter.convertType(void 0, converter.checker.getTypeOfSymbolAtLocation(symbol, symbol.valueDeclaration));
        return new M.SignatureParameterType(symbol.name,
