 ...
 */
export function makeToKindArray(upperBound: number): (kind: number) => number[] {
    return kind => {
        const kinds: number[] = [];
 ...
 */
export function makeToKindString(enumObj: Record<number, string | number>): (kind: number) => string {
    return kind => {
        assert(Number.isInteger(Math.log2(kind)), `Tried to get a kind string for kind ${kind}, which is not an exact kind.`);
