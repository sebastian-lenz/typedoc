 ...
 */
export type ThemeRouterConstructor = new (project: ProjectReflection) => ThemeRouter;
...
export class ThemeRouter {
    ...
    createLink(from: Reflection, to: Reflection): string {
        ...
        return posix.relative(posix.dirname(fromPage), toPage) + (slug ? `#${slug}` : '');
    }
    ...
    createAssetLink(from: Reflection, asset: string): string {
        ...
        return posix.relative(reflectionDir, posix.join(this.getAssetDirectory(), asset));
    }
    ...
    getDocumentName(reflection: Reflection): string {
        ...
        while (docReflection !== this.project) {
            ...
            assert(docReflection.parent, 'Cannot create a document name for a reflection not in a project.');
            docReflection = docReflection.parent;
        }

        if (this.hasOwnDocument(reflection) && this._anyChildrenHaveOwnDocument(reflection)) {
            // Our children will be placed under our directory, so we should take index.html.
            return [...parts, 'index.html'].join('/');
        }

        const fileName = `${ReflectionKind.toKindString(pageReflection.kind)}.${this._getSafeFilename(pageReflection.name)}.html`;
