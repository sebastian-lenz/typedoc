...
export class ReferenceType extends Type {
  ...
  get reflection(): Reflection | undefined {
    ...
    for (const reflection of this._project.getReflectionsFromSymbol(
      this._reference
    )) {
      if (
        this._preferValueSpace !== reflection.kindOf(TYPE_CONTRIBUTING_KINDS)
      ) {
  ...
  constructor(
    ...
    this._reference =
      reference instanceof Reflection ? reference.id : reference;
  ...
  serialize(
    ...
    init: BaseSerialized<ReferenceType>
...
export interface SerializedReferenceType
  extends Serialized<ReferenceType, "name" | "typeArguments"> {
  target?: number;
}
