...
import { convertTypeParameters, convertParameters, hasReadonlyModifier, hasQuestionToken } from './utils';

export interface TypeNodeConverter<T extends ts.TypeNode = ts.TypeNode, O extends M.SomeType = M.SomeType> {
    kind: T['kind'][];
...
// T[]
const arrayTypeNodeConverter: TypeNodeConverter<ts.ArrayTypeNode, M.ArrayType> = {
...
const exprWithTypeArgsTypeNodeConverter: TypeNodeConverter<ts.ExpressionWithTypeArguments, M.ReferenceType> = {
    ...
    convert(converter, node) {
        ...
        assert(targetSymbol, `Reference type failed to get a symbol for ${node.expression.getText()}. This is probably a bug.`);
...
const referenceTypeNodeConverter: TypeNodeConverter<ts.TypeReferenceNode, M.ReferenceType> = {
    ...
    convert(converter, node) {
        ...
        assert(targetSymbol, `Reference type failed to get a symbol for ${node.typeName.getText()}. This is probably a bug.`);
        const parameters = node.typeArguments?.map(node => converter.convertType(node)) ?? [];
        return new M.ReferenceType(targetSymbol.name, parameters, targetSymbol, false, converter.project);
    }
}
...
const typeLiteralConverter: TypeNodeConverter<ts.TypeLiteralNode, M.ObjectType | M.SignatureType | M.ConstructorType> = {
    ...
    convert(converter, node, type) {
        const properties = node.members
            ...
            .map(prop => new M.PropertyType(getPropertyName(prop.name),
                hasReadonlyModifier(prop),
                hasQuestionToken(prop),
                converter.convertType(prop.type, converter.checker.getTypeAtLocation(prop))));

        const signatures = node.members
            .filter(ts.isCallSignatureDeclaration)
            .map(node => new M.SignatureType(
                convertTypeParameters(converter, node.typeParameters ?? []),
                convertParameters(converter, node.parameters),
                converter.convertType(node.type)));
        ...
        const constructSignatures = node.members
            ...
            .map(node => new M.ConstructorType(
                convertTypeParameters(converter, node.typeParameters ?? []),
                convertParameters(converter, node.parameters),
                converter.convertType(node.type)));
...
function getLiteralValue(value: ts.BooleanLiteral | ts.LiteralExpression | ts.PrefixUnaryExpression): string | number | boolean | undefined {
    switch (value.kind) {
